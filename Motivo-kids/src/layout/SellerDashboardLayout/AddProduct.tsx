...
const AddProductPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-6">
      <div className="bg-white dark:bg-gray-800 p-8 md:p-12 rounded-3xl shadow-2xl w-full max-w-3xl">
        <h2 className="text-3xl md:text-4xl font-extrabold mb-8 text-gray-800 dark:text-gray-100 text-center">
          Add New Product
        </h2>
<form
  onSubmit={handleSubmit}
  className="bg-white dark:bg-gray-800 p-6 md:p-10 rounded-3xl shadow-2xl w-full max-w-3xl mx-auto grid gap-4"
>
  {/* Category */}
  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
    <label className="md:w-32 text-gray-700 dark:text-gray-300 font-semibold">
      Category:
    </label>
    <select
      value={categoryId}
...
<div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
  ...
  <input
    type="number"
    value={rating || 0}  // make sure value is always a number
    onChange={(e) => {
      let val = parseFloat(e.target.value);
      if (isNaN(val)) val = 0; // fallback to 0
      if (val > 5) val = 5; // max 5
